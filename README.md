...
Run `npm test` to execute the Vitest suite. It covers client reconnection and backoff, connection cleanup, message forwarding, command sending, and server heartbeat handling for responsive and stale sockets.
...
- `server/index.js` fetches each actual driving route from the public OSRM demo server. It keeps data in memory, uses Turf `length` and `along` to road-snap every current position, and broadcasts the fleet through `ws`. Protocol-level heartbeats remove stale sockets, while the client reconnects with bounded exponential backoff.
...
This demo intentionally has no traffic data, dynamic rerouting, durable data persistence, authentication, vehicle editor, or mobile layout.
